...
- 不对探针训练集做过采样，保持原始类分布；训练时使用类别权重（balanced）增强有害类学习
...
def main():
    ...
    results = train_layer_probes(
        ...
        train_indices=train_indices,  # 使用全部训练集（不过采样）
        ...
        use_class_weight=True,               # 方案B：类别权重（balanced），不过采样
    )
