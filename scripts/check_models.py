...
def main():
    ...
    # 检查备用路径
    check_model_path(LLM_WORKSPACE_PATH, "推理模型（容器内 - /workspace/ms_models）")
    check_model_path(GUARD_WORKSPACE_PATH, "安全分类器（容器内 - /workspace/ms_models）")
    ...
    # 检查 /workspace/ms_models
    workspace_models = Path("/workspace/ms_models")
    if workspace_models.exists():
        print(f"✓ /workspace/ms_models 存在")
        ...
        print(f"✗ /workspace/ms_models 不存在")
        print(f"  提示: 如果模型在 F 盘，请确保启动容器时使用 -v F:/DC25/part1/NeuroBreak-Reproduction/ms_models:/workspace/ms_models")
