...
def main():
    ...
    print(f"[Final] 探针训练: {len(probe_train_texts)}, 探针验证: {len(probe_val_texts)}")
    ...
    print("[Hidden] 提取探针训练集隐藏态（使用平均池化）...")
    train_hs_list = extract_hidden_states(
        ...
        pooling_method="mean",  # 使用平均池化，包含整个序列的信息
    )
    print("[Hidden] 提取探针验证集隐藏态（使用平均池化）...")
    ...
    print(f"       训练: {train_hs.shape}, 验证: {val_hs.shape}, 约 {args.output.stat().st_size / 1024 / 1024:.1f} MB")
    print()
